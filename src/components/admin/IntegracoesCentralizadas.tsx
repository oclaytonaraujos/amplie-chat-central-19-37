...
export default function IntegracoesCentralizadas() {
  ...
  const loadWebhooks = async () => {
    try {
      ...
      const webhooksSimulados: WebhookConfig[] = [
        {
          id: '1',
          nome: 'Z-API Empresa A',
          tipo: 'zapi',
          url: 'https://api.z-api.io/instances/ABC123/webhook',
          status: 'ativo',
          empresa_id: 'emp1',
  ...
  const getTipoBadge = (tipo: string) => {
    const configs = {
      zapi: { color: 'bg-blue-100 text-blue-800', icon: MessageSquare },
  ...
  const webhooksFiltrados = webhooks.filter(webhook => {
    const matchTipo = !filtroTipo || webhook.tipo === filtroTipo;
    const matchStatus = !filtroStatus || webhook.status === filtroStatus;
